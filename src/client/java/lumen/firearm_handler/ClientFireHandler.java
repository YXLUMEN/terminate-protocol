...
import static lumen.firearm_handler.GunHandler.RECOIL_SYSTEM;
import static lumen.terminate_protocol.item.AbstractGunItem.getMuzzleOffset;
...
public class ClientFireHandler {
    ...
    public static void onFireTick(ClientPlayerEntity player, ItemStack itemStack) {
        if (itemStack.getDamage() >= itemStack.getMaxDamage() ||
                player.getItemCooldownManager().isCoolingDown(itemStack.getItem())) return;

        long currentTime = System.currentTimeMillis();
        AbstractGunItem item = (AbstractGunItem) itemStack.getItem();
        if (currentTime - lastFireTime < item.getFireRant() * 50L) return;
        ...
        spawnMuzzleFlash(player, muzzlePos, lookVec);
        ...
        RECOIL_SYSTEM.applyRecoil(vertical, horizontal);
